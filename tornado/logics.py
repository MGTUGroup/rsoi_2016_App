...
class TokenHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        try:
            client = tornado.httpclient.AsyncHTTPClient()
            response = yield tornado.gen.Task(client.fetch, url_for_session + 'id', method='POST',
                                              headers={"Content-Type": "application/json; charset=UTF-8"},
            ...
            if response.code == 404:
                ...
                token = json.loads(response.body)['token']
                self.write(json.dumps(dict(status=200, token=token)))
        except:
            self.send_error(response.error)
        self.finish()
    ...
    def add_taxi_to_order(self, pass_id, taxi_id):
        try:
            ...
            response = client.fetch(url_for_order + 'add_taxi', method='POST',
												headers={"Content-Type": "application/json; charset=UTF-8"},
                                                body=simplejson.dumps(dict(pass_id=pass_id, taxi_id=taxi_id)))
            self.set_status(200)
        except:
            self.send_error(response.error)
...
class Start_calculationHandler(Change_statusHandler):
    ...
    @tornado.gen.engine
    def post(self):
        taxi_id = tornado.escape.json_decode(self.request.body)['taxi_id']
        try:
            client = tornado.httpclient.AsyncHTTPClient()
            response = yield tornado.gen.Task(client.fetch, url_for_order + 'start_calculation', method='POST',
											    headers={"Content-Type": "application/json; charset=UTF-8"},
                                                body=simplejson.dumps(dict(taxi_id=taxi_id)))
            if response.code == 200:
                self.change_status('taxi', 'transit', taxi_id)
                self.write(json.dumps(dict(status=200)))
            else:
                self.send_error(500)
            self.finish()
        except:
...
class Stop_calculationHandler(Change_statusHandler):
    @tornado.web.asynchronous
    @tornado.gen.engine
    def post(self):
        taxi_id = tornado.escape.json_decode(self.request.body)['taxi_id']
